            ...
            <div className="flex items-center gap-2 relative">
              {user ? (
                <>
                  ...
                  >
                    <span className="text-[10px] font-bold text-slate-600 hidden sm:block">{user.name}</span>
                    <img src={user.avatar_url} alt="Google" className="w-7 h-7 rounded-full border border-white shadow-sm" />
                  </button>
                  ...
                  {showAccountMenu && (
                    <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-2xl z-[100] animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                      <div className="p-4 border-b border-slate-100 bg-slate-50/80">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-1.5"><UserCircle size={10} /> Conta Google</p>
                      ...
                      <div className="p-1">
                        <button 
                          ...
                          disabled={isLoading}
                          className="w-full flex items-center gap-3 px-3 py-2.5 text-xs text-red-600 hover:bg-red-50 rounded-xl transition-colors font-semibold disabled:opacity-50"
                        >
                          <LogOut size={14} /> {isLoading ? 'Saindo...' : 'Sair do Perfil'}
                        </button>
                      </div>
                    </div>
                  )}
                ...
                <button 
                  ...
                  disabled={isLoading}
                  className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-3 py-2 rounded-lg transition-all text-[10px] font-bold shadow-md active:scale-95 disabled:opacity-50"
                >
